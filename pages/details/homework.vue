...
export default {
  ...
  async asyncData ({route, store, error}) {
    ...
    try {
      await store.dispatch('homework/appendHomeworkDetails', { id: route.query.id }).then((res) => {
        if (res && res.data) {
  ...
  created () {
    if (process.browser && !this.isServiceload) {
      this.$store.dispatch('homework/appendHomeworkDetails', { id: this.$route.query.id }).then((res) => {
        if (res && res.data) {
