      ...
      <template v-if="!userLikesGetters.life.list.length && finished">
        <div class="blank-box">
          <div class="posts-blank-wrap">
            ...
            <span class="blank-txt">暂无喜欢生活动态</span>
          </div>
        </div>
      </template>
    </template>
    <template v-if="pageName === 'userCollection'">
      ...
      <template v-if="!userFavoritesGetters.life.list.length && finished">
        <div class="blank-box">
          <div class="posts-blank-wrap">
            <img class="blank-icon" :src="dynamic_Blank" alt="" />
            <span class="blank-txt">暂无收藏生活动态</span>
          </div>
        </div>
      </template>
    </template>
  ...
  methods:{
    ...
    onLoad() {
      if(this.pageName === 'userLike') {
        ...
        const newPage = this.userLikesGetters.life.pageInfo.pages + 1
        this.appendUserLikes({
          type: 'LIFE',
          page: newPage,
          size: 10
        ...
        const newPage = this.userFavoritesGetters.life.pageInfo.pages + 1
        this.appendUserFavorites({
          type: 'LIFE',
          page: newPage,
          size: 10
